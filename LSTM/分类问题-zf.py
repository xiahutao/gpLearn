...
if __name__ == '__main__':
    ...
    pdxycsv = pd.read_csv(data_path + 'data8.csv', index_col=0, header=0)  # 把csv文件里的内容读出来
    data = torch.tensor(pdxycsv.values, dtype=torch.float)  # 把xy数据转化成pytorch的tensor格式
    test_data_count = 500  # 最后留100个数据作为测试用
    xtrain = data[0:-2*test_data_count, 0:-2]  # 把1000个数据中的900个作为训练的数据x
    # xtrain = guiyimaxmin(xtrain)  # 把数据归一化一下
    ytrain = data[0:-2*test_data_count, [-1]]  # 把1000个数据中的900个作为训练的数据y
    xtest = data[-2*test_data_count:-test_data_count, 0:-2]  # 把1000个数据中的最后100个作为测试数据x
    # xtest = guiyimaxmin(xtest)  # 把数据归一化一下
    ytest = data[-2*test_data_count:-test_data_count, [-1]]  # 把1000个数据中的最后100个作为测试数据

    xpredict = data[-test_data_count:, 0:-2]  # 把1000个数据中的最后100个作为验证数据x
    # xtest = guiyimaxmin(xtest)  # 把数据归一化一下
    ypredict = data[-test_data_count:, [-1]]  # 把1000个数据中的最后100个作为测试数据
    # ypre_value = data[-test_data_count:, [-2]]
    ypre_value = pdxycsv['ret'].tolist()[-test_data_count:]
    print(ypre_value)
    ...
    if train_use == 0:
        ...
        ax2 = plt.subplot(132)
        ax3 = plt.subplot(133)
        for epoch in range(400):  # 循环训练200次
            model.train()  # 切换到训练模式
    ...
    if train_use != 1:
        ...
        ret = pd.DataFrame({'predict': b, 'true': y_true_value, 'ret': y_ret_value})
        print(len(ret))
        ret = ret[ret['predict'] != 1]
        print(len(ret))
        ...
        acc_s = len(ret[(ret['acc'] == 0) & (ret['ret_y'] == 2)]) / len(ret[ret['ret_y'] == 2])
        acc_b = len(ret[(ret['acc'] == 0) & (ret['ret_y'] == 0)]) / len(ret[ret['ret_y'] == 0])
        print('acc：%s, acc_bs：%s, acc_s：%s, acc_b：%s' %(acc, acc_bs, acc_s, acc_b))
    if train_use == 1:
        timeold = ''
        while (True):
