...
class HmmStrategy():
    def __init__(self, leverage, fee, test_df, cols_features, hmm_model, long_states,
                 random_states, short_states, **kwargs):
        super(HmmStrategy, self).__init__(leverage=leverage,
                                          fee=fee,
                                          test_df=test_df,
    ...
    def _handle_data(self):
        ...
        short_stop = False
        state_lst = []
        for idx, _row in self.test_df.iterrows():
            test_data = [_row[i] for i in self.cols_features]
            state = int(self.model.predict(test_data))
            state_lst.append(state)
            print('State on ' + str(idx) + ': ' + str(state))
            if pos == 0:
                if state in self.long_states:
                    pos = 1
                ...
                elif state in self.short_states:
                    cost = _row.open * (1 - fee)
                    pos = -1
                    s_time = _row.date_time
                    hold_price = []
                ...
                if state in self.short_states:
                    ...
                    short_stop = False
                    long_stop = False
                elif state in self.random_states:
                    s_price = _row.open * (1 - fee)
                    trad_times += 1
                    net1 = pos * s_price / _row.close_1 + (1 - pos)
                    ret = s_price / cost - 1
            ...
            net_lst.append(net)
            pos_lst.append(pos)
        ret = self.test_df[['close', 'date_time']]
        ret['net'] = net_lst
        ret['pos'] = pos_lst
        ret['state'] = state_lst
        self.ret = ret
        self.signal_state_all = pd.DataFrame(signal_lst, columns=[
    ...
    def _analyze(self):
        ...
        print('annR: ' + str(annR))
        print('sharp: ' + sharp)
        print('Max drawdown: ' + max_retrace)

        f = plt.figure(figsize=(7.2, 7.2))

        # Plot return
        ax1 = f.add_subplot(211)
        ax1.plot(ret.net, 'blue')
        ax1.plot(ret.net_close, 'red')
        ax1.legend()
