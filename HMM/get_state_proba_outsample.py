...
if __name__ == '__main__':
    ...
    for asset in asset_lst:
        ...
        for feature_lst in all_feature_lst:
            ...
            train_set = net_new.loc[:'2020-01-01']
            len_train = len(train_set)
            test_set = net_new.loc['2020-01-01':]
            X = np.column_stack([train_set.loc[:, feature_lst]])
            # X = X.reshape(-1, 1)
            ...
            for i in range(len(test_set)):
                X_ = net_new.head(len_train + i + 1)
                X_ = np.column_stack([X_.loc[:, feature_lst]])
                predic = model.predict_proba(X_)[-1]
                predic_lst.append(list(predic))
            predic_df = pd.DataFrame(predic_lst, columns=['state%s' %i for i in range(6)])
            test_set = test_set.reset_index(drop=True)
            print(test_set)
            test_set = pd.concat([test_set, predic_df], axis=1)
            print(test_set)
            plt.figure(figsize=(20, 10))
            test_set = test_set.set_index(['date_time'])
            test_set = test_set.loc['2020-01-01':]
            ...
            test_set[['state%s' %i for i in range(6)]].plot()
