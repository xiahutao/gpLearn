    ...
    for asset, group in param_all.groupby(['asset']):
        ...
        for i in range(len(train_days_lst)):
            ...
            strategy_obj = HmmStrategy(
                ...
                cols_features=factor_lst, price_name='close', type='future', train_days=train_day,
                test_days=test_day)
            annR, sharp, max_retrace, ret_df = strategy_obj.run_outsample()
            print(asset, '特征因子：', factor_lst, '训练窗口:', train_day, '测试窗口:', test_day, '夏普', sharp)
            ret_df.to_csv(resualt_path + 'net_param/%s_%s_%s.csv' %(asset, train_day, test_day), encoding='gbk')
            # net_df = ret_df[['date_time', 'chg']].rename(columns={'chg': 'chg' + str(i)}).reset_index(drop=True)
            # net_df = pd.read_csv(
